public class CountryDiplomacy {
    public double Friendship = 50;
    public bool TradeAgreement = false;
    public bool BeingSpied = false;
}

public class ArmyStuff {
    public int Soldiers = 0;
    public int Ammo = 0;
    public int InfantryWeapons = 0;
    public int ArmoredCars = 0;
    public int CombatVehicles = 0;
    public int Artillery = 0;
    public int RPG = 0;
    public int BallisticMissiles = 0;
    public int Tanks = 0;
    public int MassDestruction = 0;
}

public class NavyStuff {
    public int Sailors = 0;
    public int Ammo = 0;
    public int PatrolBoats = 0;
    public int Amphibious = 0;
    public int BallisticMissiles = 0;
    public int Submarines = 0;
    public int AircraftCarrier = 0;
    public int Warship = 0;
    public int Destroyer = 0;
    public int MassDestruction = 0;
}

public class AirForceStuff {
    public int Pilots = 0;
    public int Ammo = 0;
    public int Surveillance = 0;
    public int Reconnaissance = 0;
    public int Helicopters = 0;
    public int AttackAircraft = 0;
    public int FighterPlanes = 0;
    public int Bombers = 0;
    public int MassDestruction = 0;
}