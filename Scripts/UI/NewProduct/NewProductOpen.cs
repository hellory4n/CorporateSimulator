...
public class NewProductOpen : TextureButton {
    ...
    public void Click() {
        if (GetNodeOrNull("/root/Developing") == null) {
            if (Global.Employees.Count > 0) {
                switch (Global.Industries[0]) {
                    ...
                    case "phones":
                        ...
                        break;
                    case "yourarse":
                        var yesat3am = (PackedScene)ResourceLoader.Load("res://Scenes/NewProducts/YourArse.tscn");
                        Node2D OKat3am = (Node2D)yesat3am.Instance();
                        OKat3am.ZIndex = 100;
                        GetTree().Root.AddChild(OKat3am);
                        break;
                    case "games":
                        var yesGAMING = (PackedScene)ResourceLoader.Load("res://Scenes/NewProducts/Games.tscn");
                        Node2D OKGAMING = (Node2D)yesGAMING.Instance();
                        OKGAMING.ZIndex = 100;
                        GetTree().Root.AddChild(OKGAMING);
                        break;
                }
            } else {
            ...
            GetNode<Label>("Label").Text = "You can't make 2 products at the same time!";
        }
    }
}
